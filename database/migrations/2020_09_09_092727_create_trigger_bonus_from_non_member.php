...
class CreateTriggerBonusFromNonMember extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        DB::unprepared('DROP TRIGGER IF EXISTS `tr_bonus_sponsor_from_non_member_insert`');
        ...
        CREATE TRIGGER tr_bonus_sponsor_from_non_member AFTER UPDATE ON `transaction_non_members` 
            FOR EACH ROW BEGIN
                ...
                SET @is_promo = (SELECT count(id) FROM `transaction_members_promotion` WHERE `member_id` = NEW.non_member_id AND `ebook_id` = NEW.ebook_id AND type = "nonmember");
                ...
                IF @is_promo > 0 THEN
                    ...
                    SET @bonus_bv = @bonus_bv - (@bonus_bv * (@discount / 100));
                    SET @bonus_pv = @bonus_pv - (@bonus_pv * (@discount / 100));
                    DELETE FROM `transaction_members_promotion` WHERE `member_id` = NEW.non_member_id AND `ebook_id` = NEW.ebook_id AND type = "nonmember";
                END IF;
                IF @verif = 0 THEN
                    set @pajak = 0.03;
                ELSE
        ...
        DB::unprepared('
        CREATE TRIGGER tr_bonus_sponsor_from_non_member_insert AFTER INSERT ON `transaction_non_members` 
            FOR EACH ROW BEGIN
                SET @bonus_bv = (SELECT bv FROM `ebooks` WHERE id = NEW.ebook_id);
                SET @is_promo = (SELECT count(id) FROM `transaction_members_promotion` WHERE `member_id` = NEW.non_member_id AND `ebook_id` = NEW.ebook_id AND type = "nonmember");
                SET @bonus_pv = (SELECT pv FROM `ebooks` WHERE id = NEW.ebook_id);
